...
if ($context->options['mobile'] === true) {
    ...
    $page->maincontentarea = '<div class="four_col">' . $savvy->render($context->output) . '<a id="reportProblem" href="http://www1.unl.edu/comments/">Have a correction?</a><div id="commentProblem"><h3>Have a correction?</h3><form id="wdn_feedback_comments2" method="post" action="http://www1.unl.edu/comments/"><input type="hidden" id="page_address" value="" /><textarea name="comment" id="comment" rows="" cols=""></textarea><input type="submit" value="Submit" /></form></div></div>';
}
